...
for (fold in 1:K) {
  ...
  verosimiFeatClase <- lapply(namesFeatures, function(x) verIndFC(dataTraining[,1],dataTraining[,x],c("clase",x)))
  
  names(verosimiFeatClase) <- namesFeatures
  
  Prob_clase<-table(dataTraining$clase)
  Prob_clase <- Prob_clase/dim(dataTraining)[1]
  nrows <- nrow(dataTest)
  Result_Test <- sapply(seq(1:nrows), function(y) sapply(namesFeatures, function(x) verosimiFeatClase[[x]][valClases[1],dataTest[y,][[x]]]))
  Result_Test <- t(Result_Test)
  Result_Test_prod <- sapply(seq(1:nrows), function(x) prod(Result_Test[x,])*Prob_clase[1])
  
  
  Result_Test2 <- sapply(seq(1:nrows), function(y) sapply(namesFeatures, function(x) verosimiFeatClase[[x]][valClases[2],dataTest[y,][[x]]]))
  Result_Test2 <- t(Result_Test2)
  Result_Test2_prod <- sapply(seq(1:nrows), function(x) prod(Result_Test2[x,])*Prob_clase[2])
  ...
  dim(claseEst)
  dim(dataTest)
  TablaFinal <- data.frame(PpostCe=Result_Test_prod,PpostCp=Result_Test2_prod,claseEst$clase,dataTest$clase)
  TablaFinal$claseEst.clase <- factor(TablaFinal$claseEst.clase)
  
  TP <- table(TablaFinal$claseEst.clase,TablaFinal$dataTest.clase)
  sumRows_TP<-rowSums(TP)
  sumCols_TP<-colSums(TP)
  
  Exactitud <- c(Exactitud,(TP[1,1]*TP[2,2])/sum(sumRows_TP))
  
  Error <-  c(Error,(TP[1,2]*TP[2,1])/sum(sumRows_TP))
  
  Recall <-  c(Recall,(TP[1,1])/sumRows_TP[1])
  
  Especificidad <-  c(Especificidad,(TP[2,2])/sumRows_TP[2])
  
  Presicion <-  c(Presicion,(TP[1,1])/TP[1,1]+TP[2,1])
  
  F_Score <- c(F_Score,2*Presicion[fold]*Recall[fold]/Presicion[fold]*Recall[fold])
