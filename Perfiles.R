...
count <- 0
dist_matrix <- proxy::dist(datasetKnnPerfiles [, c("Horas.Semana.Divertirse.con.sus.amigos", "Materias.Aprobadas.Primer.Semestre", "Edad")], method = "Gower")
...
predictClass <- function(Horas.Semana.Divertirse.con.sus.amigos, Materias.Aprobadas.Primer.Semestre, Edad) {
  # Crear el dataframe con los valores de entrada
  input_df <- data.frame(Horas.Semana.Divertirse.con.sus.amigos = factor(Horas.Semana.Divertirse.con.sus.amigos,
                                                 levels = levels(datasetKnnPerfiles$Horas.Semana.Divertirse.con.sus.amigos)),
                         Materias.Aprobadas.Primer.Semestre = factor(Materias.Aprobadas.Primer.Semestre,
                                         levels = levels(datasetKnnPerfiles$Materias.Aprobadas.Primer.Semestre)),
                         Edad = factor(Edad,
                                              levels = levels(datasetKnnPerfiles$Edad)))
  
  # Calcular la matriz de distancia de Gower para el fold de prueba
  test_dist_matrix <- proxy::dist(input_df, datasetKnnPerfiles[, c("Horas.Semana.Divertirse.con.sus.amigos", "Materias.Aprobadas.Primer.Semestre", "Foto_Perfil")], method = "Gower")
  
  # Realizar el clasificador KNN utilizando la distancia de Gower
  k <- 3  # Número de vecinos
  prediction <- knn(train = dist_matrix, test = test_dist_matrix, cl = datasetKnnPerfiles$claseFace, k = k)
...
server <- function(input, output){
  observeEvent(input$predictButton, {
    Horas.Semana.Divertirse.con.sus.amigos <- input$Horas.Semana.Divertirse.con.sus.amigos
    Materias.Aprobadas.Primer.Semestre <- input$Materias.Aprobadas.Primer.Semestre
    Edad <- input$Edad
    
    prediction <- predictClass(Horas.Semana.Divertirse.con.sus.amigos, Materias.Aprobadas.Primer.Semestre, Edad)
    count <- count + 1
    print(count)
